...
class _WorkLogFormState extends State<WorkLogForm> {
  ...
  DateTime _workStartTime = DateTime.now(); // Default to current time for clock-in
  ...
  void initState() {
    ...
    if (widget.details.isNotEmpty) {
      ...
      _workStartTime = DateTime.parse(widget.details['workStartTime'] ?? DateTime.now().toIso8601String());

      if (widget.details['bookingRequests'] != null) {
        _selectedBookingRequests = List<int>.from(widget.details['bookingRequests'].map((b) => b['id']));
      }
    }
  }
  ...
  void _submitForm() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final response = await http.post(
        Uri.parse('${getWorkLogUrl()}/create'), // Ensure correct endpoint and method
        headers: headers,
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['status'] == 200) {
          ...
          throw Exception('Failed to create work log: ${jsonResponse['message']}');
        }
      } else {
        throw Exception('Failed to create work log. Status code: ${response.statusCode}');
      }
    }
  }
